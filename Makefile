...
run_variant_indexer:
	docker run -it -v `pwd`:/usr/src/ansible/ ${LOCAL_RUN_CONTAINER}:${TAG} ansible-playbook -e env=${ENV} -i hosts launch_variant_launch_indexer.ymlindexer.yml --vault-password-file=.password
